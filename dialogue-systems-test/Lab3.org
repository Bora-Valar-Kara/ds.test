...
3. Create a *Resource group* (you can use search field):
   ...
   - Resource group: *RG-GU* (or any name)
...
4. Create a *Speech service* (you can use search field):
   ...
   - Resource group: *RG-GU* (or the same group name from the previous step)
...
1. Install [[https://nodejs.org/en/download/][NodeJS]] (LTS version) and [[Download and https://yarnpkg.com/getting-started/install][Yarn]] if you haven't already.
2. Fork the example project (this project):
   https://github.com/GU-CLASP/dialogue-systems-1-2026. Clone your
...
5. Create a file called ~azure.ts~ in ~src/~ folder:
   ...
   cat .gitignore
   #+end_src
   Add azure.ts file with its relative location to ".gitignore" file.
   #+begin_src sh
   echo "src/azure.ts" >> .gitignore
   #+end_src
   (Note: Using echo command with single ">" will overwrite the file! As a rule of thumb, be very careful when using this command! For example, =echo "src/azure.ts" > .gitignore= will overwrite everything inside that file!)
6. Test the project:
   #+begin_src sh
   yarn dev
   #+end_src
