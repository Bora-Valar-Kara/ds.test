...
5. Create a file called ~azure.ts~ in ~src/~ folder:
   ...
   #+end_src
   (Note: Don't add this file to version control. ".gitignore" file does it for you. Check where that file is located using =cd= and =ls -la= commands and see which files are not added to version control. Add azure.ts file with its relative location to ".gitignore" file.)
   #+begin_src sh
